...
class SecondExampleViewController: UIViewController {
    ...
    private lazy var MainViewCollectionView: UICollectionView = {
        ...
        collectionView.register(HeaderView.self, forSupplementaryViewOfKind: SecondExampleViewController.sectionHeaderElementKind, withReuseIdentifier: HeaderView.reuseIdentifier)
        collectionView.register(
            GaleryCollectionViewCell.self,
            forCellWithReuseIdentifier: GaleryCollectionViewCell.id
        )
    ...
    private func generateRecommendedTourLayout() -> NSCollectionLayoutSection {
        ...
        let sectionHeader = NSCollectionLayoutBoundarySupplementaryItem(
          ...
          elementKind: SecondExampleViewController.sectionHeaderElementKind,
          alignment: .top)
    ...
    private func configureDataSource() {
        ...
        let carouselItems = mock.mockForCarousel // Пример данных для секции "carouselTour"
        snapshot.appendItems(carouselItems.map { Item.carousel($0) }, toSection: .carouselTour)

        let galeryItems: [tourModel] = mock.mockForGalery
        snapshot.appendItems(galeryItems.map { Item.galery($0) }, toSection: .galeryTour)

        let recommendedItems: [tourModel] = mock.mockForRecommended
...
extension SecondExampleViewController: UICollectionViewDelegate {
